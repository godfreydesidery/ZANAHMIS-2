...
@Component({
  selector: 'app-slow-moving-drugs',
  templateUrl: './slow-moving-drugs.component.html',
  styleUrls: ['./slow-moving-drugs.component.scss'],
  standalone : true,
  imports : [
    CommonModule,
    FormsModule,
...
export class SlowMovingDrugsComponent {
  address  : any 
  ...
  logo!    : any
  documentHeader! : any
  slowMovingProducts : IDrugMovement[] = []
  async loadFastMovingDrugsReport(from : Date, to : Date){
    ...
    this.spinner.show()
    await this.http.post<IDrugMovement[]>(API_URL+'/reports/slow_moving_drugs_report', args, options)
    ...
    .then(
      data => {
        ...
        data?.forEach(element => {
            this.slowMovingProducts.push(element)
        })
        var sn = 1
        this.slowMovingProducts.forEach(element => {
          element.sn = sn
          sn = sn + 1
        })
      }
  ...
  print = async () => {
    ...
    if(this.slowMovingProducts.length === 0){
      this.msgBox.showErrorMessage3('No data to print')
      return
    }

    this.documentHeader = await this.data.getDocumentHeader()
    var title  = 'Slow Moving Drugs Report'
    var total : number = 0
