...
import { IPharmacyStockCard } from 'src/app/domain/pharmacy-stock-card';
...
@Component({
  selector: 'app-pharmacy-stock-card-report',
  templateUrl: './pharmacy-stock-card-report.component.html',
  styleUrls: ['./pharmacy-stock-card-report.component.scss'],
  standalone : true,
  imports : [
    CommonModule,
    FormsModule,
...
export class PharmacyStockCardReportComponent {
  ...
  logo!    : any
  documentHeader! : any
  pharmacyStockCards : IPharmacyStockCard[] = []
  async loadReport(from : Date, to : Date){
    ...
    this.spinner.show()
    await this.http.post<IPharmacyStockCard[]>(API_URL+'/reports/pharmacy_stock_card_report', args, options)
    .pipe(finalize(() => this.spinner.hide()))
    .toPromise()
    .then(
      data => {
        console.log(data)
        this.pharmacyStockCards = data!
        var sn = 1
        this.pharmacyStockCards.forEach(element => {
          element.sn = sn
          sn = sn + 1
        })
      }
  ...
  clear(){
    this.pharmacyStockCards = []
  }

  print = async () => {

    if(this.pharmacyStockCards.length === 0){
      this.msgBox.showErrorMessage3('No data to print')
      return
    }

    this.documentHeader = await this.data.getDocumentHeader()
    var title  = 'Pharmacy Stock Card Report'
    var total : number = 0
    ...
    this.pharmacyStockCards.forEach((element) => {
      var detail = [
        ...
        {text : element.pharmacy.name, fontSize : 6, fillColor : '#ffffff'},
