...
@Component({
  selector: 'app-doctor-follow-up',
  templateUrl: './doctor-follow-up.component.html',
  styleUrls: ['./doctor-follow-up.component.scss'],
  standalone : true,
  imports : [
    CommonModule,
    FormsModule,
...
export class DoctorFollowUpComponent {
  ...
  async saveLabTest(){
    ...
    this.spinner.show()
    await this.http.post<ILabTest>(API_URL+'/patients/save_lab_test?consultation_id='+this.id+'&non_consultation_id='+0+'&admission_id='+0, labTest, options)
    .pipe(finalize(() => this.spinner.hide()))
    .toPromise()
    .then(
      (data) => {
  ...
  async saveRadiology(){
    ...
    this.spinner.show()
    await this.http.post<IRadiology>(API_URL+'/patients/save_radiology?consultation_id='+this.id+'&non_consultation_id='+0+'&admission_id='+0, radiology, options)
    .pipe(finalize(() => this.spinner.hide()))
    .toPromise()
    .then(
      (data) => {
 ...
 fileExtension : string = ''
  ...
  async switchToConsultation(id : any){
    ...
    this.spinner.show()
    await this.http.get<IConsultation>(API_URL+'/patients/switch_to_consultation_by_consultation_id?id='+id, options)
    .pipe(finalize(() => this.spinner.hide()))
    .toPromise()
    .then(
      data => {
    ...
    .catch(
      error => {
        this.msgBox.showErrorMessage(error, 'Could not switch to consultation')
        console.log(error)
      }
    )
  }
