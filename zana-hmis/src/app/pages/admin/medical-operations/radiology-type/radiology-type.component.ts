...
export class RadiologyTypeComponent implements OnInit {
  ...
  exportTemplateToExcel() {
    ...
    worksheet.columns = [
      { header: 'RADIOLOGY_TYPE_CODE', key: 'RADIOLOGY_TYPE_CODE'},
      { header: 'RADIOLOGY_TYPE_NAME', key: 'RADIOLOGY_TYPE_NAME'},
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Radiology Type Master Template.xlsx');
    })
   
  }

  async exportRadiologyTypesToExcel() {
    let workbook = new Workbook();
    ...
    .then(
      data => {
        data?.forEach(element => {         
          worksheet.addRow(
            {
              RADIOLOGY_TYPE_CODE          : element.code,
              RADIOLOGY_TYPE_NAME          : element.name,
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      fs.saveAs(blob, 'Radiology Types.xlsx');
    })
   
  }

  async uploadRadiologyTypesFile(evt: any) {
    var radiologyTypeData: [][]
    ...
    reader.onload = (e: any) => {
      ...
      radiologyTypeData = (XLSX.utils.sheet_to_json(ws, { header: 1 }))

      this.progress = true
      if (this.validateRadiologyTypeMaster(radiologyTypeData) == true) {
        this.progressStatus = 'Uploading... please wait'
        this.uploadRadiologyType(radiologyTypeData)
      } else {
        alert('Invalid Radiology type file')
      }
      this.progress = false
    }
  ...
  updateMedicineFile(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      if (this.validateRadiologyTypeMaster(radiologyTypeData) === true) {
        ...
        this.updateRadiologyTypes(radiologyTypeData)
      } else {
        alert('Invalid radiology type file')
      }
      this.progress = false
  ...
  validateRadiologyTypeMaster(data : any [][]) : boolean{
    this.clearProgress()
    ...
    //validate row header
    if( data[0][0] != 'RADIOLOGY_TYPE_CODE' ||
        data[0][1] != 'RADIOLOGY_TYPE_NAME' ||
  ...
  async updateRadiologyTypes(dt : any [][]){
    let options = {
      headers: new HttpHeaders().set('Authorization', 'Bearer '+this.auth.user.access_token)
    }
    this.clearProgress()
    ...
    this.progressStatus = 'Updating radiology types file'
    this.totalRecords = rows
    ...
    for(let i = 1; i < dt.length; i++) {
      ...
      this.spinner.show()
      await this.http.put(API_URL+'/radiology_types/update_by_code', radiologyType, options)
      .pipe(finalize(() => this.spinner.hide()))
      .toPromise()
      .catch(
        error => {
  ...
  async uploadRadiologyType(dt : any [][]){
    let options = {
      headers: new HttpHeaders().set('Authorization', 'Bearer '+this.auth.user.access_token)
    }
    this.clearProgress()
    var rows = dt.length
    var cols = dt[0].length
    this.progressStatus = 'Uploading radiology type file'
    this.totalRecords = rows
    ...
    for(let i = 1; i < dt.length; i++) {
      ...
      this.spinner.show()
      await this.http.post(API_URL+'/radiology_types/save', medicine, options)
      .pipe(finalize(() => this.spinner.hide()))
      .toPromise()
      .catch(
        error => {
